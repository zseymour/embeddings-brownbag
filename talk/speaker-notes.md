...
4. The fair production-serving comparison (own e12 script) — single vector, ColBERTv2-style compression, PLAID, and MUVERA on the same corpus, at production-realistic storage and latency. The main path uses e11 for the quality signal and e12 for the fair compressed-storage comparison.
...
- Late interaction: ColBERT (arXiv:2004.12832), ColBERTv2 (arXiv:2112.01488), Token Pooling (arXiv:2409.14683), ColPali (arXiv:2407.01449), MUVERA (arXiv:2405.19504), LIR workshop at ECIR 2026 (arXiv:2511.00444). The main path keeps one model through the serving bridge: ColPali represents a page as 1,024 patch vectors (257.5 KB), then its own 3× hierarchical pooling ablation retains 97.8% retrieval performance with 33% of the vectors. Pooling keeps a multi-vector representation; MUVERA changes only the candidate shortlist to a fixed-dimensional MIPS encoding, then exact MaxSim reranks the candidates. Vector-DB support verified per vendor docs; see references.md.
